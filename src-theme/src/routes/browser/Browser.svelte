...
{#if browser}
    <div class="browser-controls">
        ...
        <div class="address-bar">
            <input id="url" bind:value={browser.url} on:keypress={onKeyPress} placeholder="输入网址" />
        </div>
        <button on:click={handleGo}>前往</button>
        <button on:click={handleForceReload}>强制刷新</button>
    </div>
{/if}
