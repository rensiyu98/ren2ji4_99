            ...
            {#if filteredModules.length > 0}
                {#each filteredModules as module, index (module.name)}
                    ...
                    <div
                            ...
                            on:click={() => toggleModule(module.name, !module.enabled)}
                            on:contextmenu|preventDefault={() => $highlightModuleName = module.name}
                    ...
                    >
                        ...
                        <div class="aliases">
                            {#if module.aliases.length > 0}
                                (aka {module.aliases.map(alias => displayName(alias)).join(", ")})
                            {/if}
                        </div>
                    </div>
                {/each}
