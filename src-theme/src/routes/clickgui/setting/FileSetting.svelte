<script lang="ts">
    ...
    import {displayName} from "../../../theme/theme_config";
...
<div class="setting">
    <div class="name">{displayName(cSetting)}</div>
    ...
    <div class="body">
        ...
        {#if cSetting.value !== ""}
            <button class="button-action" on:click={resetFile}>
                <img class="icon" src="img/clickgui/icon-reset.svg" alt="重置文件" title="重置" />
            </button>

            <button class="button-action" on:click={() => browsePath(cSetting.value)}>
                <img class="icon" src="img/clickgui/icon-open-file.svg" alt="打开文件" title="打开" />
            </button>
        {/if}
    </div>
</div>
