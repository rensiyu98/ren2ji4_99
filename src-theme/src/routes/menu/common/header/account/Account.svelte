    ...
    <div class="header" bind:this={headerElement}>
        ...
        <div class="buttons">
            <button class="icon-button" type="button" on:click={loginWithRandomUsername}>
                ...
                <img class="icon" src="img/menu/account/icon-random.svg" alt="随机用户名">
            </button>
            <button class="icon-button" disabled={inAccountManager} type="button"
                    on:click={() => openScreen("altmanager")}>
                <ToolTip text="切换账户"/>

                <img class="icon" src="img/menu/icon-pen.svg" alt="切换账户">
            </button>
        </div>
    </div>
    ...
    {#if expanded}
        <div class="quick-switcher" transition:fade|global={{ duration: 200, easing: quintOut }}>
            ...
            {#if accounts.length > 0}
                ...
                {:else}
                    <div class="placeholder">无结果</div>
                {/if}
            {:else}
                <div class="placeholder">账户列表为空</div>
            {/if}
        </div>
    {/if}
</div>
