...
open class ClientModule @JvmOverloads constructor(
    ...
    /**
     * Modules translate through their own namespace, so that a word shared with a
     * setting can read differently in each — see [moduleNameTranslation].
     */
    override val translatedName: String
        get() = moduleNameTranslation(name)
    ...
    final override fun onToggled(state: Boolean): Boolean {
        ...
        if (!loadingNow) {
            ...
            notification(title, translatedName, severity)
        }
