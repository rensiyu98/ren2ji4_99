...
open class Value<T : Any>(
     ...
     *
     * [name] itself stays English: it doubles as the config key, as the
     * `module.settings.<value>` lookup and as the add-on API identifier.
     * Only what is rendered is swapped out here, so saved configs and
     * scripts keep working regardless of the selected language.
     */
    // No backing field, so Gson never sees it — nothing to exclude, and the
    // annotations below would not compile on a computed property anyway.
    open val translatedName: String
        get() = settingNameTranslation(name)
